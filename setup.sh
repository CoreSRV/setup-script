...
preflight(){
    output "Server Setup Script ${version}"
    output "Copyright © 2021 CoreSRV <mail@coresrv.io>."
...
activate_iptables(){
    output "Activate basic IPtable rules?"
    output "[1] Yes (Recommended)."
    output "[2] No."
    read iptableschoice
    case $iptableschoice in
    1 ) output "Activating IPtables"
        curl -sSL https://raw.githubusercontent.com/coresrv/setup-script/master/iptables.sh | bash
        block_icmp
        ;;
...
activate_tuned(){
    output "Activate Tuned Low-Latency profile?"
    output "[1] Yes (Recommended)."
    output "[2] No."
    read finetune
    case $finetune in
...
activate_fail2ban(){
    output "Setup Fail2Ban to protect sshd?"
    output "[1] Yes."
    output "[2] No."
    read f2b 
    case $f2b in 
...
whitelist_cloudflare(){
    output "Do you want to whitelist only Cloudflare IPs, on your selection of ports?"
    output "[1] Yes."
    output "[2] No."
    read cloudflare
    case $cloudflare in
    1)  bash <(curl -sSL https://raw.githubusercontent.com/coresrv/setup-script/master/cloudflare.sh)
        ;;
    2) output "Skipping..."
        ;;
    * ) output "You did not enter a valid selection."
...
whitelist_tcpshield(){
    output "Do you want to whitelist only TCPShield IPs, on your selection of ports?"
    output "[1] Yes."
    output "[2] No."
    read tcpshield
    case $tcpshield in
    1)  bash <(curl -sSL https://raw.githubusercontent.com/coresrv/setup-script/master/tcpshield.sh)
        ;;
    2) output "Skipping..."
        ;;
    * ) output "You did not enter a valid selection."
...
javapipe_kernel(){
    output "Apply JavaPipe's kernel configurations? (Kernel DDoS Protection/)"
    output "[1] Yes."
    output "[2] No."
    read javapipe
    case $javapipe in
...
spectre_vulnerbility_check(){
     output "Run the spectre and meltdown vulnerbility check script?"
     output "[1] Yes."
     output "[2] No."
     read spectre
     case $spectre in
...
secure_ssh
motd
output "Finished Server configuration."
